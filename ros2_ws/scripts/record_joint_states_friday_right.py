...
from ros2_ws.scripts.joint_state_recorder_friday_right import (
    JointStatesRecorderFridayRight,
)
import os
import argparse
import rclpy
...
if __name__ == "__main__":
    ...
    data_folder = os.path.join(absolute_path, "data", "friday_right", save_folder_name)
    if data_folder and not os.path.exists(data_folder):
        os.makedirs(data_folder)

    rclpy.init()
    node = JointStatesRecorderFridayRight(data_folder)
    try:
        while rclpy.ok():
