    ...
    for folder_ith in folder:
        ...
        if number_samples is not None:
            for i in range(len(data[robot_name])):
                data[robot_name][i] = data[robot_name][i][
                    np.random.choice(
                        data[robot_name][i].shape[0], number_samples, replace=False
                    )
                ]
    return data


def read_data_3sockets(folder: str, number_samples: Optional[int] = None) -> None:
    # create an empty dictionary to store the data
    data = {}
    for folder_ith in folder:
        robot_name = os.path.basename(folder_ith)
        data[robot_name] = read_each_3sockets(folder_ith)
        if number_samples is not None:
            for i in range(len(data[robot_name])):
                data[robot_name][i] = data[robot_name][i][
                    np.random.choice(
...
def read_each(folder: Optional[str] = None) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if data_1.shape[1] != data_2.shape[1]:
        raise ValueError(
            f"Data files {file_path_1} and {file_path_2} have different number of columns."
        )
    return data_1, data_2


def read_each_3sockets(folder: Optional[str] = None) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if not os.path.exists(file_path_2):
        raise FileNotFoundError(f"Data file {file_path_2} not found.")
    file_path_3 = os.path.join(recording_folder, "hole_2.csv")
    if not os.path.exists(file_path_3):
        raise FileNotFoundError(f"Data file {file_path_3} not found.")
    ...
    try:
        ...
        raise ValueError(f"Failed to load data from {file_path_2}. Error: {e}")
    try:
        data_3 = np.loadtxt(file_path_3, delimiter=",")
    except Exception as e:
        raise ValueError(f"Failed to load data from {file_path_3}. Error: {e}")
    # check if the array is loaded correctly and it is not empty
    if data_1.size == 0 or data_2.size == 0 or data_3.size == 0:
        raise ValueError(
            f"Data files {file_path_1} or {file_path_2} or {file_path_3} are empty."
        )
    if data_1.shape[1] != data_2.shape[1]:
        raise ValueError(
            f"Data files {file_path_1} and {file_path_2} have different number of columns."
        )
    if data_1.shape[1] != data_3.shape[1]:
        raise ValueError(
            f"Data files {file_path_1} and {file_path_3} have different number of columns."
        )
    if data_2.shape[1] != data_3.shape[1]:
        raise ValueError(
            f"Data files {file_path_2} and {file_path_3} have different number of columns."
        )
...
def overlay_images(images: list, output_path: str):
    ...
    assert all(
        img.shape[0] == height and img.shape[1] == width for img in images
...
def remove_outliers(model, data, verbose=False):
    ...
    for tool_position, recorded_joints in data.items():
        ...
        iqr_1 = q3_1 - q1_1
        mask_1 = np.all(
            (fks_1 >= (q1_1 - alpha * iqr_1)) & (fks_1 <= (q3_1 + alpha * iqr_1)),
            axis=1,
        )
        ...
        iqr_2 = q3_2 - q1_2
        mask_2 = np.all(
            (fks_2 >= (q1_2 - alpha * iqr_2)) & (fks_2 <= (q3_2 + alpha * iqr_2)),
            axis=1,
        )
        if verbose:
            ...
            print(
                f"Outliers detected for hole 0: {len(fks_1) - np.sum(mask_1)} for tool position {tool_position}"
            )
            print(
                f"Outliers detected for hole 1: {len(fks_2) - np.sum(mask_2)} for tool position {tool_position}"
            )
...
def evaluate_model_3sockets(
    model: URDF,
    data_folder: List[str],
    verbose: bool = False,
) -> Dict[str, np.ndarray]:
    data = read_data_3sockets(data_folder)
    # remove outliers
    data = remove_outliers_3sockets(model, data)
    statistics = {}
    for tool_position, recorded_joints in data.items():
        data_hole_0 = recorded_joints[0]
        data_hole_1 = recorded_joints[1]
        data_hole_2 = recorded_joints[2]
        fks_1 = np.zeros((len(data_hole_0), 3))
        fks_2 = np.zeros((len(data_hole_1), 3))
        fks_3 = np.zeros((len(data_hole_2), 3))
        for i, joint_angles in enumerate(data_hole_0):
            joint = np.zeros(model.num_actuated_joints)
            joint[: len(joint_angles)] = joint_angles
            model.update_cfg(joint)
        ...
        for i, joint_angles in enumerate(data_hole_1):
            ...
            fk = model.get_transform(frame_from=model.base_link, frame_to="ball_link")
            fks_2[i] = np.array(fk[:3, 3]).flatten()
        for i, joint_angles in enumerate(data_hole_2):
            joint = np.zeros(model.num_actuated_joints)
            joint[: len(joint_angles)] = joint_angles
            model.update_cfg(joint)
            fk = model.get_transform(frame_from=model.base_link, frame_to="ball_link")
            fks_3[i] = np.array(fk[:3, 3]).flatten()

        fk_mean_1 = np.mean(fks_1, axis=0)
        N_1 = len(fks_1)
        fk_mean_2 = np.mean(fks_2, axis=0)
        N_2 = len(fks_2)
        fk_mean_3 = np.mean(fks_3, axis=0)
        N_3 = len(fks_3)
        fk_variance_1 = np.sum(np.var(fks_1, axis=0))
        fk_variance_2 = np.sum(np.var(fks_2, axis=0))
        fk_variance_3 = np.sum(np.var(fks_3, axis=0))
        # average_absolute_error
        fks_1_average = np.mean(np.linalg.norm(np.abs(fks_1 - fk_mean_1), axis=1))
        fks_2_average = np.mean(np.linalg.norm(np.abs(fks_2 - fk_mean_2), axis=1))
        fks_3_average = np.mean(np.linalg.norm(np.abs(fks_3 - fk_mean_3), axis=1))
        weighted_average = (
            N_1 * fks_1_average + N_2 * fks_2_average + N_3 * fks_3_average
        ) / (N_1 + N_2 + N_3)
        std_dev_fk = np.sqrt(
            fk_variance_1 + fk_variance_2 + fk_variance_3
        )  # this is not the actual error
        mean_squared_error = (
            fk_variance_1 * N_1 + fk_variance_2 * N_2 + fk_variance_3 * N_3
        ) / (N_1 + N_2 + N_3)
        std_dev_fk = np.sqrt(mean_squared_error)
        statistics[tool_position] = {
            "mean_1": fk_mean_1.tolist(),
            ...
            "mean_2": fk_mean_2.tolist(),
            "std_dev_2": float(np.sqrt(fk_variance_2)),
            "mean_3": fk_mean_3.tolist(),
            "std_dev_3": float(np.sqrt(fk_variance_3)),
            "fks_1": fks_1.tolist(),
            "fks_2": fks_2.tolist(),
            "fks_3": fks_3.tolist(),
            "mean_absolute_error": float(weighted_average),
...
def remove_outliers_3sockets(model, data, verbose=False):
    ...
    for tool_position, recorded_joints in data.items():
        ...
        q1_3 = np.percentile(fks_3, 25, axis=0)
        q3_3 = np.percentile(fks_3, 75, axis=0)
        iqr_3 = q3_3 - q1_3
        mask_3 = np.all(
            (fks_3 >= (q1_3 - alpha * iqr_3)) & (fks_3 <= (q3_3 + alpha * iqr_3)),
            axis=1,
        )
        if verbose:
            ...
            print(
                f"Outliers detected for hole 1: {len(fks_3) - np.sum(mask_3)} for tool position {tool_position}"
            )
...
def compute_improved_performance(
    model_folder: str,
    data_folders_train: List[str],
    data_folders_test: List[str],
    offset_distance,
    latex=False,
) -> None:
    ...
    for step in steps:
        ...
        statistics_train = evaluate_model(
            model_step, data_folders_train, offset_distance=offset_distance
        )
        ...
        statistics_test = evaluate_model(
            model_step, data_folders_test, offset_distance=offset_distance
        )
    ...
    percentage_improved_distance_train = [
        (distances_train[i][0] - distances_train[i][-1]) / distances_train[i][0] * 100
        for i in range(len(data_folders_train))
    ]
    percentage_improved_variance_train = [
        (variances_train[i][0] - variances_train[i][-1]) / variances_train[i][0] * 100
        for i in range(len(data_folders_train))
    ]
    percentage_improved_mae_train = [
        (mae_train[i][0] - mae_train[i][-1]) / mae_train[i][0] * 100
        for i in range(len(data_folders_train))
    ]
    print(
        f"The mean absolute error  went from {np.mean(np.array(mae_train)[:,0]):.2e} to {np.mean(np.array(mae_train)[:,-1]):.2e} on the train data on average"
    )
    ...
    print(
        f"Percentage of removed error on train set: {np.mean(percentage_improved_mae_train):.2f}"
    )
    ...
    if distances_test:  # if there is test data
        percentage_improved_distance_test = [
            (distances_test[i][0] - distances_test[i][-1]) / distances_test[i][0] * 100
            for i in range(len(data_folders_test))
        ]
        percentage_improved_variance_test = [
            (variances_test[i][0] - variances_test[i][-1]) / variances_test[i][0] * 100
            for i in range(len(data_folders_test))
        ]
        percentage_improved_mae_test = [
            (mae_test[i][0] - mae_test[i][-1]) / mae_test[i][0] * 100
            for i in range(len(data_folders_test))
        ]

        print(
            f"The mean absolute error  went from {np.mean(np.array(mae_test)[:,0]):.2e} to {np.mean(np.array(mae_test)[:,-1]):.2e} on the test data on average"
        )
        ...
        print(
            f"Percentage of removed error on test set: {np.mean(percentage_improved_mae_test):.2f}"
        )
...
def modify_urdf(
    urdf_file: str, search_pattern: str, replace_with: str, output_file: str
):
    """
...
def check_urdf_path(urdf_path: str):
    if not os.path.exists(urdf_path):
        ...
        parent_directory = os.path.join(
            os.path.dirname(script_directory), os.path.pardir
        )
...
def check_model_path(model_folder: str):
    if not os.path.exists(model_folder):
        ...
        print(
            f"model {model} does not exist in calibrated_urdf. Did you already run the optimizer?"
        )
        # Check if the data folder exists and the urdf path exists, otherwise exit
        script_directory = os.path.abspath(__file__)
        parent_directory = os.path.join(
