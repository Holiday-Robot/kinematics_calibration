...
from calibrate_fk.utils import (
    evaluate_model_3sockets,
    read_data_3sockets,
...
class ParameterOptimizer:
    ...
    def set_offset_distance(self, offset_distance):
    ...
    def select_parameters(
        ...
        if selected_parameters is None:
            ...
            for joint in self._symbolic_fk.robot.active_joints():
                joint_is_active = input(
                    f"Should the joint {joint} be optimized? (y/n): "
                )
    ...
    def create_parameters(
        self, selected_joints: List[str], variance_noise: float = 0.0
    ) -> None:
        self._params = {}
        self._best_params = {}
        ...
        for joint in root.findall("joint"):
            if joint.get("name") == joint_name:
                ...
                if origin is not None:
                    ...
                    return [float(val) for val in xyz_values.split()] + [
                        float(val) for val in rpy_values.split()
                    ]
        raise ValueError(f"Joint {joint_name} not found in URDF file.")

    def create_fk_expression(self) -> None:
        self._fk_casadi_expr = self._symbolic_fk.casadi(
            self._q,
            self._end_link,
            position_only=True,
            symbolic_parameters=self._params,
        )
        self._fk_casadi_expr_pure = self._symbolic_fk.casadi(
            self._q, self._end_link, position_only=True
        )
    ...
    def read_data(self, folder: str, number_samples: Optional[int] = None) -> None:
        ...
        self.data = read_data_3sockets(folder=folder, number_samples=number_samples)

    def optimize(self, saving_steps: bool = False):
        self.create_fk_expression()
        self._fk_fun_pure = ca.Function(
            "fk_pure", [self._q], [self._fk_casadi_expr_pure]
        )
        # for each element in read_multiplethe dictionary
        objective = 0

        if len(self.data) * 2 != len(self._offset_distance):
            raise ValueError(
                f"Data Number: {len(self.data)}, Offset Distance Nuimber: {len(self._offset_distance)} => Data Number x 2 should be same with Offset Distance Nuimber"
            )
        for i, sockets in enumerate(self.data.values()):
            fks_1 = []
            for joint_angles_1 in sockets[0]:
                substituted_fk = ca.substitute(
                    self._fk_casadi_expr, self._q, joint_angles_1
                )
                fks_1.append(substituted_fk)
            fks_2 = []
            for joint_angles_2 in sockets[1]:
                substituted_fk = ca.substitute(
                    self._fk_casadi_expr, self._q, joint_angles_2
                )
                fks_2.append(substituted_fk)
            fks_3 = []
            for joint_angles_3 in sockets[2]:
                substituted_fk = ca.substitute(
                    self._fk_casadi_expr, self._q, joint_angles_3
                )
                fks_3.append(substituted_fk)

            fk_mean_1 = ca.sum2(ca.horzcat(*fks_1)) / len(fks_1)
            N1 = len(fks_1)
            fk_variance_1 = ca.sum2(
                ca.horzcat(*[(fk - fk_mean_1) ** 2 for fk in fks_1])
            ) / len(fks_1)
            fk_variance_norm_1 = ca.sum1(fk_variance_1)

            fk_mean_2 = ca.sum2(ca.horzcat(*fks_2)) / len(fks_2)
            N2 = len(fks_2)
            fk_variance_2 = ca.sum2(
                ca.horzcat(*[(fk - fk_mean_2) ** 2 for fk in fks_2])
            ) / len(fks_2)
            fk_variance_norm_2 = ca.sum1(fk_variance_2)

            fk_mean_3 = ca.sum2(ca.horzcat(*fks_3)) / len(fks_3)
            N3 = len(fks_3)
            fk_variance_3 = ca.sum2(
                ca.horzcat(*[(fk - fk_mean_3) ** 2 for fk in fks_3])
            ) / len(fks_3)
            fk_variance_norm_3 = ca.sum1(fk_variance_3)

            distance_error1 = (
                ca.norm_2(fk_mean_1 - fk_mean_2) - self._offset_distance[2 * i]
            )
            distance_error2 = (
                ca.norm_2(fk_mean_2 - fk_mean_3) - self._offset_distance[2 * i + 1]
            )
            distance_error3 = ca.norm_2(fk_mean_1 - fk_mean_3) - (
                self._offset_distance[2 * i] + self._offset_distance[2 * i + 1]
            )

            objective += (
                (
                    fk_variance_norm_1 * N1
                    + fk_variance_norm_2 * N2
                    + fk_variance_norm_3 * N3
                )
                / (N1 + N2 + N3)
                + distance_error1**2
                + distance_error2**2
        ...
        objective += (
            ...
            * self._regulizer_weight
            * ca.sum1(ca.vertcat(*residuals))
        )
        ...
        calibration_iteration_callback = IterationStepCallback(
            "iteration_callback", nx=nx
        )
        solver_options = {
            "ipopt": {
                "print_level": 0,
        ...
        if saving_steps:
            for i, intermediate_solution in enumerate(
                calibration_iteration_callback.solutions
            ):
                self.save_intermediate_solution(
                    intermediate_solution, parameter_list, i
                )

    def save_intermediate_solution(
        self, intermediate_solution: List[float], parameter_list: List[ca.SX], i: int
    ):
        ...
        kpis = evaluate_model_3sockets(
            intermediate_model, self._data_folder, verbose=False
        )
    ...
    def evaluate_fks(self, verbose: bool = False) -> dict:
        kpis = evaluate_model_3sockets(self._model, self._data_folder, verbose=verbose)
