...
from calibrate_fk.parameter_optimizer_holiday import ParameterOptimizer
from calibrate_fk.utils import (
    check_urdf_path,
    check_data_path,
    remove_outliers_3sockets,
)
...
def main():
    ...
    argument_parser.add_argument(
        "--model", "-m", help="Name of the urdf stored in the urdf folder."
    )
    argument_parser.add_argument(
        "--data",
        "-t",
    ...
    argument_parser.add_argument(
        ...
        type=float,
        nargs="+",
        help="Distance between the sockets in meters, defauls it 0.05",
    )
    argument_parser.add_argument(
        "--regularizer", "-reg", help="Regularizer for the optimization", default=1e-4
    )
    argument_parser.add_argument(
        "--end-effector", "-ee", help="End effector link", default="ball_link"
    )
    argument_parser.add_argument(
        "--root-link", "-rl", help="Root link", default="base_link"
    )
    argument_parser.add_argument(
        "--variance_noise",
        "-v",
        help="Variance of the noise injected to the initial robot parameters",
        default=0.00,
    )
    argument_parser.add_argument(
        "--number-samples", "-n", help="Number of samples to use", default=None
    )
    ...
    output_path = os.path.abspath(
        os.path.join(parent_directory, "calibrated_urdf", robot_name)
    )
    ...
    if os.path.exists(output_path):
        print(
            f"Output folder {output_path} already exists. Please delete it or specify a different folder."
        )
        sys.exit(1)
    ...
    optimizer.select_parameters(
        variance_noise=variance_noise, selected_parameters=parameters["joints"]
    )
    optimizer.data = remove_outliers_3sockets(optimizer._model, optimizer.data)
